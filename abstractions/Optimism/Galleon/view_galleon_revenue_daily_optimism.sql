...
dune_user_generated."galleon_revenue_daily" as
...
from        (select
            ...
            from        dune_user_generated."galleon_issuance_daily" a
            left join   (select distinct
                            date_trunc('day', hour) as day,
                            symbol,
                            percentile_cont(.5) within group (order by median_price) as price
                        from        prices."prices_from_dex_data"
                        where       contract_address in (select token_address from dune_user_generated.galleon_tokens)
                        group by    day, symbol
                        ) b on a.day = b.day and a.symbol = b.symbol
            ) a
left join   dune_user_generated."galleon_fee_structure" b on a.symbol = b.symbol and a.day >= b.begin_date and a.day <= b.end_date
